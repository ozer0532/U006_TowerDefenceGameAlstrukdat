	...
	for (int i = 0; i <= p.NBrsEff;i ++){
		for (int j = 0; j <= p.NKolEff; j ++){
			if ((i == 0)||(j == 0)||(i == p.NBrsEff)||(j == p.NKolEff)){
				...
				if (p.Mem[i][j].C == 'e') printf("  ");
				else 
				{
					if (p.Mem[i][j].p == 0) printf("%c ",p.Mem[i][j].C);
                    else if (p.Mem[i][j].p == 1) {print_blue(("%c",p.Mem[i][j].C));printf(" ");}
                    else if (p.Mem[i][j].p == 2) {print_red(("%c",p.Mem[i][j].C));printf(" ");}
				}
			}
